...
with st.sidebar:
    ...
    alpha        = st.slider("Severity alpha", min_value=0.0, max_value=0.9,
                              step=0.1, value=0.3)
...
def load_and_train(dataset_name):
    ...
    mlp, _  = train_mlp(X_train, y_train, input_dim=input_dim, epochs=150)
    tabt, _ = train_tabtransformer(X_train, y_train, input_dim=input_dim, epochs=200)
...
if run_button:
    ...
    for col, method in zip(method_cols, METHODS):
        ...
        alarmed = p < 0.05
        col.markdown(f"**{METHOD_LABELS[method]}**")
        if alarmed:
    ...
    for ax, method in zip(axes, METHODS):
        ...
        ax.hist(corr_unc[method],     bins=20, alpha=0.6, label=f"a={alpha}", color="#e07070")
    ...
    for method in METHODS:
        ...
        rows.append({
            ...
            "Alarm alpha":     alarm_alpha,
            "Acc Drop alpha":  drop_alpha,
    ...
    if drop_alpha is not None:
        ax2.axvline(drop_alpha, linestyle=":", color="red", label=f"Drop a={drop_alpha}")
    ...
    3. **Set severity alpha** — how aggressively to corrupt the test set
    4. **Run** — see which uncertainty method raises the alarm earliest

    ### Uncertainty Methods
    | Method | Type | Approach |
    |--------|------|----------|
    | **Conformal (MAPIE)** | Classical | Coverage-guaranteed prediction sets via split conformal |
    | **NGBoost Entropy** | Classical | Full predictive distribution, uncertainty via binary entropy |
    | **MC Dropout MLP** | Deep Learning | Two-layer MLP with Bayesian dropout at inference time |
    | **TabTransformer** | Deep Learning | Self-attention over features + MC dropout uncertainty |
    """)
